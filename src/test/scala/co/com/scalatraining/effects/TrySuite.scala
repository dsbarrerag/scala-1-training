...
class TrySuite extends FunSuite with Matchers {
  ...
  test("Un Failure se debe poder recuperar con recover") {
    ...
    val res = f.map(x => "HOLA")
      .recover { case e: Exception => "HOLA ME HE RECUPERADO"
      }

    assert(res == Success("HOLA ME HE RECUPERADO"))
    res.flatMap(s =>
      Try(assert(s == "HOLA ME HE RECUPERADO")))
  }

  test("Error en la recuperacion con recover") {
    val rec = f.recover {
      case _: Exception => throw new Exception("Mi nuevo error")
    }
    assert(rec.isFailure)

    rec match {
      case Failure(e) => assert(e.getMessage === "Mi nuevo error")
    }
  }


  test("Error en la recuperacion con recoverWith") {
    val rec = f.recoverWith {
      case _: Exception => Try(throw new Exception("Mi nuevo error"))
    }
  ...
  test("Un Failure se debe poder recuperar con recoverWith") {
    ...
    val res = f.map(x => "HOLA")
      .recoverWith { case e: Exception => s
      }
  ...
  test("Recover Failure inside for-com") {

    val res = for {
      x <- s
      y <- f.recoverWith {
        case e: Exception => Try(2)
      }
      z <- s
    } yield x + y + z
  ...
  test("Recover Failure outside for-com") {
    ...
    val res = (
      ...
      case e: Exception => Try(99)
    }

    assert(res === Success(99))
